...
describe("loadConfigFile", () => {
  ...
  it.skipIf(process.platform === "win32")("throws when the file is world-writable on POSIX", async () => {
    const root = await tempRoot();
    roots.push(root);
    const filePath = path.join(root, "env.json");
  ...
  it.skipIf(process.platform !== "win32")(
    "loads a Windows-style file without applying POSIX mode-bit hardening",
    async () => {
      const root = await tempRoot();
      roots.push(root);
      const filePath = path.join(root, "env.json");
  ...
  it.skipIf(process.platform === "win32" || process.getuid?.() === 0)(
    "wraps read failures after a successful stat in EnvValidationError",
    async () => {
      const root = await tempRoot();
      roots.push(root);
